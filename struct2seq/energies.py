...
class PairEnergies(nn.Module):
    def __init__(self, num_letters, node_features, edge_features, input_dim,
        ...
        """
        N = 3
        self.cnn = nn.Conv3d(1, 20, kernel_size = (1, 1, N), padding = (0, 0, N//2))
        self.W_out = nn.Linear(hidden_dim, 20)
        """
    ...
    def forward(self, V_embed, X, x_mask, sparse = False):
        ...
        for layer in self.encoder_layers:
            h_EV = layer(h_EV, h_EV, E_idx, mask_E=x_mask, mask_attend=mask_attend)

        h_EV = self.W_out(h_EV)#/30
        ...
        """
        h_E = self.cnn(h_E.unsqueeze(1))
        h_E = self.W_out(h_E).transpose(1,3).transpose(1,2)
        n_batch, L, k = h_E.shape[:3]
        h_E = h_E.contiguous().view(n_batch, L, k, -1)
        #print(h_E.shape)
        """
