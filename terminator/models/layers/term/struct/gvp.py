...
from terminator.models.layers.utils import gather_nodes, gather_term_nodes, cat_gvp_term_edge_endpoints
...
class TERMGVPEncoder(nn.Module):
    def __init__(self, hparams):
        ...
        self.encoder_layers = nn.ModuleList([
            layer(vec_in=self.nv + self.ev,
                  num_hidden=hidden_dim,
                  dropout=dropout) for _ in range(num_encoder_layers)
        ])

        self.W_out = GVP(vi=self.ev,
                         vo=self.ev,
                         si=self.hs,
                         so=self.hs,
                         nls=None,
    ...
    def forward(self, V, E, E_idx, mask):
        ...
        for layer in self.encoder_layers:
            h_EV = cat_gvp_term_edge_endpoints(h_E, h_V, E_idx, self.nv,
                                               self.ev)
...
class TERMGraphGVPEncoder(nn.Module):
    def __init__(self, hparams):
        ...
        node_features = (hparams['term_hidden_dim'] // 2,
                         hparams['term_hidden_dim'] // 2)
        edge_features = (hparams['term_hidden_dim'] // 2,
                         hparams['term_hidden_dim'] // 2)
        hidden_dim = hparams['term_hidden_dim']
        hidden_features = (hparams['term_hidden_dim'] // 2,
                           hparams['term_hidden_dim'] // 2)
        dropout = hparams['transformer_dropout']
        num_encoder_layers = hparams['term_layers']
    ...
    def forward(self, V, E, E_idx, mask):
        ...
        for edge_layer, node_layer in zip(self.edge_encoder,
            ...
            h_EV_nodes = cat_gvp_term_edge_endpoints(h_E, h_V, E_idx, self.nv,
                                                     self.ev)
            h_V = node_layer(h_V,
                             h_EV_nodes,
                             mask_V=mask,
            ...
            h_EV_edges = cat_gvp_term_edge_endpoints(h_E, h_V, E_idx, self.nv,
                                                     self.ev)
            h_E = edge_layer(h_E,
                             h_EV_edges,
                             mask_E=mask_attend,
