...
class SelfEnergies(Struct2Seq):
    def __init__(self,
        ...
        super(SelfEnergies, self).__init__(num_letters,
                                           node_features,
                                           edge_features,
                                           hidden_dim,
                                           num_encoder_layers=3,
...
class RawSelfEnergies(Struct2Seq):
    def __init__(self,
        ...
        super(RawSelfEnergies, self).__init__(num_letters,
                                              node_features,
                                              edge_features,
                                              hidden_dim,
                                              num_encoder_layers=3,
...
class PairEnergies(nn.Module):
    def __init__(self, hparams):
        ...
        self.features = S2SProteinFeatures(
            node_features=hdim,
            edge_features=hdim,
        ...
        # Embedding layers
        self.W_v = nn.Linear(hdim + hparams['energies_input_dim'],
                             hdim,
                             bias=True)
        self.W_e = nn.Linear(hdim, hdim, bias=True)
        layer = EdgeTransformerLayer if not hparams[
            'energies_use_mpnn'] else EdgeMPNNLayer
    ...
    def forward(self, X, x_mask, V_embed=[], sparse=False):
        ...
        if not sparse:
            ...
            h_i_idx = E_idx[:, :, 0].unsqueeze(-1).expand(-1, -1,
                                                          k).unsqueeze(-1)
            h_j_idx = E_idx.unsqueeze(-1)
            batch_num = torch.arange(n_batch).view(n_batch, 1, 1, 1).expand(
                -1, n_nodes, k, -1)
            ij = torch.cat([batch_num, h_i_idx, h_j_idx], dim=-1)

            flat_ij = ij.view(-1, 3).transpose(0, 1)
            flat_h_E = h_E.view(-1,
                                self.hparams['energies_output_dim']).float()
...
class AblatedPairEnergies(nn.Module):
    def __init__(self, hparams):
        ...
        self.W = nn.Linear(hparams['energies_input_dim'] * 2,
                           hparams['energies_output_dim'])
...
class AblatedPairEnergies_g(nn.Module):
    def __init__(self, hparams):
        ...
        self.k_neighbors = hparams['k_neighbors']
        self.W = nn.Linear(hparams['energies_input_dim'] * 3,
                           hparams['energies_output_dim'])
...
class MultiChainPairEnergies_g(PairEnergies):
    def __init__(self, hparams):
        ...
        self.W_e = nn.Linear(hdim + hparams['energies_input_dim'],
                             hdim,
                             bias=True)
...
class PairEnergiesFullGraph(nn.Module):
    def __init__(self, hparams):
        ...
        self.W_e = nn.Linear(hdim + hparams['energies_input_dim'],
                             hdim,
                             bias=True)
        edge_layer = EdgeTransformerLayer if not hparams[
            'energies_use_mpnn'] else EdgeMPNNLayer
        node_layer = TransformerLayer if not hparams[
            'energies_use_mpnn'] else NodeMPNNLayer
    ...
    def forward(self, V_embed, E_embed, X, x_mask, chain_idx, sparse=False):
        ...
        if self.hparams['energies_input_dim'] != 0:
            ...
            if not self.hparams[
                    'use_coords']:  # this is hacky/inefficient but i am lazy
                V = torch.zeros_like(V)
                E = torch.zeros_like(E)

            if torch.isnan(V).any() or torch.isnan(E).any() or torch.isnan(
                    E_idx).any():
                raise RuntimeError(
                    "nan found during struct2seq feature generation")
