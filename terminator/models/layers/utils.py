...
def batchify(batched_flat_terms, term_lens):
    ...
    list_terms = [
        torch.split(flat_terms[i], term_lens[i]) for i in range(n_batches)
    ]
...
def gather_pairEs(pairEs, neighbor_idx):
    # Features [B,N,N,C] at Neighbor indices [B,N,K] => Neighbor features [B,N,K,C]
    n_aa = pairEs.size(-1)
    neighbors = neighbor_idx.unsqueeze(-1).unsqueeze(-1).expand(
        -1, -1, -1, n_aa, n_aa)
...
def cat_term_edge_endpoints(h_edges, h_nodes, E_idx):
    ...
    h_i_idx = E_idx[:, :, :, 0].unsqueeze(-1).expand(-1, -1, -1,
                                                     k).contiguous()
    h_j_idx = E_idx
...
def cat_gvp_neighbors_nodes(h_nodes, h_neighbors, E_idx, nv_nodes,
                            nv_neighbors):
    h_nodes = gather_nodes(h_nodes, E_idx)
...
def cat_gvp_edge_endpoints(h_edges, h_nodes, E_idx, n_node, n_edge):
    ...
    # output features [B, N, K, 3C]
    h_nn = vs_concat(vs_concat(h_i, h_j, n_node, n_node), e_ij, n_node * 2,
                     n_edge)
    return h_nn
...
def merge_duplicate_edges(h_E_update, E_idx):
    ...
    merged_E_updates = torch.where(reverse_E_update != 0,
                                   (h_E_update + reverse_E_update) / 2,
                                   h_E_update)
...
def merge_duplicate_term_edges(h_E_update, E_idx):
    ...
    # collect edges into NxN tensor shape
    collection = torch.zeros(
        (n_batch, n_terms, n_aa, n_aa, hidden_dim)).to(dev)
    neighbor_idx = E_idx.unsqueeze(-1).expand(-1, -1, -1, -1,
                                              hidden_dim).to(dev)
...
def merge_duplicate_pairE_dense(h_E, E_idx):
    ...
    neighbor_idx = E_idx.unsqueeze(-1).unsqueeze(-1).expand(
        -1, -1, -1, n_aa, n_aa).to(dev)
...
def aggregate_edges(edge_embeddings, E_idx, max_seq_len):
    ...
    # collect edges into NxN tensor shape
    collection = torch.zeros(
        (n_batch, max_seq_len, max_seq_len, hidden_dim)).to(dev)
    # edge the edge indecies
    self_idx = E_idx[:, :, :, 0].unsqueeze(-1).expand(-1, -1, -1, n_neighbors)
    neighbor_idx = E_idx
    # tensor needed for accumulation
    layer = torch.arange(n_batch).view([n_batch, 1, 1,
                                        1]).expand(neighbor_idx.shape).to(dev)
    # thicc index_put_
    collection.index_put_((layer, self_idx, neighbor_idx),
                          edge_embeddings,
                          accumulate=True)
    ...
    count.index_put_((layer, self_idx, neighbor_idx),
                     count_idx,
                     accumulate=True)
...
def stat_cuda(msg):
    ...
    print('--', msg)
    print(
        'allocated 1: %dM, max allocated: %dM, cached: %dM, max cached: %dM' %
    ...
    print(
        'allocated 2: %dM, max allocated: %dM, cached: %dM, max cached: %dM' %
    ...
    print(
        'allocated 3: %dM, max allocated: %dM, cached: %dM, max cached: %dM' %
...
def inf_nan_hook_fn(self, input, output):
    ...
    if has_large(input[0]):
    ...
    elif (output == float('inf')).any() or (
            output == float('-inf')).any() or torch.isnan(output).any():
        ...
        try:
            if is_nan_inf(self.running_mean) or is_nan_inf(self.running_var):
                with open(ERROR_FILE, 'w') as fp:
                    fp.write('Inf/nan is from ' + repr(self) + ' forward\n')
                    fp.write('Inputs from b_idx 0 channel 0' +
                             repr(input[0][0][0]) + '\n')
                    fp.write('Outputs from b_idx 0 channel 0' +
                             repr(output[0][0]) + '\n')
                    fp.write('Weights ' + repr(self.weight) + '\n')
                    fp.write('Biases ' + repr(self.bias) + '\n')
                    fp.write('Running mean ' + repr(self.running_mean) + '\n')
...
def is_nan_inf(output):
    return (output == float('inf')).any() or (
        output == float('-inf')).any() or torch.isnan(output).any()
