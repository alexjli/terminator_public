...
class TERMinator(nn.Module):
    def __init__(self, hparams, device='cuda:0'):
        ...
        if self.hparams["use_terms"]:
            self.hparams['energies_input_dim'] = self.hparams[
                'term_hidden_dim']
        else:
        ...
        print(self.bot.hparams['term_hidden_dim'],
              self.top.hparams['energies_hidden_dim'])
    ...
    def forward(self, msas, features, seq_lens, focuses, term_lens,
                src_key_mask, X, x_mask, sequence, max_seq_len, ppoe):
        etab, E_idx = self.potts(msas, features, seq_lens, focuses, term_lens,
                                 src_key_mask, X, x_mask, max_seq_len, ppoe)
    ...
    def potts(self, msas, features, seq_lens, focuses, term_lens, src_key_mask,
              X, x_mask, max_seq_len, ppoe):
        if self.hparams['use_terms']:
            condense = self.bot(msas, features, seq_lens, focuses, term_lens,
                                src_key_mask, max_seq_len, ppoe)
    ...
    def opt_sequence(self, msas, features, seq_lens, focuses, term_lens,
                     src_key_mask, X, x_mask, sequences, max_seq_len, ppoe):
        etab, E_idx = self.potts(msas, features, seq_lens, focuses, term_lens,
                                 src_key_mask, X, x_mask, max_seq_len, ppoe)
    ...
    def pred_sequence(self, msas, features, seq_lens, focuses, term_lens,
                      src_key_mask, X, x_mask, max_seq_len, ppoe):
        etab, E_idx = self.potts(msas, features, seq_lens, focuses, term_lens,
                                 src_key_mask, X, x_mask, max_seq_len, ppoe)
    ...
    def percent_recovery(self, msas, features, seq_lens, focuses, term_lens,
                         src_key_mask, X, x_mask, sequences, max_seq_len,
                         ppoe):
        etab, E_idx = self.potts(msas, features, seq_lens, focuses, term_lens,
                                 src_key_mask, X, x_mask, max_seq_len, ppoe)
        self_nrgs = self._get_self_etab(etab)
    ...
    def _seq(self, etab, E_idx, x_mask, sequences):
        ...
        E_aa = torch.gather(
            sequences.unsqueeze(-1).expand(-1, -1, k - 1), 1, E_idx[:, :, 1:])
        E_aa = E_aa.view(list(E_idx[:, :, 1:].shape) + [1, 1]).expand(
            -1, -1, -1, 21, -1)
...
class MultiChainTERMinator_gcnkt(TERMinator):
    ...
    def potts(self, msas, features, seq_lens, focuses, term_lens, src_key_mask,
              X, x_mask, max_seq_len, ppoe, chain_idx, contact_idx):
        """
        ...
        if self.hparams['use_terms']:
            node_embeddings, edge_embeddings = self.bot(
                msas,
                features,
                seq_lens,
                focuses,
        ...
        etab, E_idx = self.top(node_embeddings, edge_embeddings, X, x_mask,
                               chain_idx)
    ...
    def pred_sequence(self, msas, features, seq_lens, focuses, term_lens,
                      src_key_mask, X, x_mask, max_seq_len, ppoe, chain_lens,
                      contact_idx):
        etab, E_idx = self.potts(msas,
                                 features,
                                 seq_lens,
...
class MultiChainTERMinator_gstats(TERMinator):
    ...
    def forward(self, msas, features, seq_lens, focuses, term_lens,
                src_key_mask, X, x_mask, sequence, max_seq_len, ppoe,
                chain_lens, sing_stats, pair_stats):
        etab, E_idx = self.potts(msas, features, seq_lens, focuses, term_lens,
                                 src_key_mask, X, x_mask, max_seq_len, ppoe,
                                 chain_lens, sing_stats, pair_stats)
        nlcpl, avg_prob, counter = _nlcpl(etab, E_idx, sequence, x_mask)
    ...
    def potts(self, msas, features, seq_lens, focuses, term_lens, src_key_mask,
              X, x_mask, max_seq_len, ppoe, chain_lens, sing_stats,
              pair_stats):
