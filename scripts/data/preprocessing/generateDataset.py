...
from packageTensors import dumpTrainingTensors, dumpCoordsTensors


# when subprocesses fail you usually don't get an error...
def generateDatasetParallel(in_folder, out_folder, cutoff = 50, num_cores = 1, update = True, stats = False, weight_fn = "neg", coords_only=False, dummy_terms=None):
        ...
        for idx, file in enumerate(glob.glob(folder+'/*.red.pdb')):
            ...
            if not update:
                ...
                if os.path.exists(out_file + '.features'):
                    continue
            res = pool.apply_async(dataGen, args=(file, folder, out_folder, cutoff, stats, weight_fn, coords_only, dummy_terms), error_callback = raise_error)
